...
static int xm_sample(xm_context_t *, float *, float *);
...
#define HAS_VIBRATO(s) ((s)->effect_type == 4 || (s)->effect_type == 6 || ((s)->volume_column >> 4) == 0xB)
...
static void xm_vibrato(xm_context_t *ctx, xm_channel_context_t *ch, uint8_t param)
{
	...
	ch->vibrato_note_offset =
		-2.f * xm_waveform(ch->vibrato_waveform, ch->vibrato_ticks) * (float)(param & 0x0F) / (float)0xF;
	xm_update_frequency(ctx, ch);
}
		...
		case 0xD: /* EDy: Note delay */
			...
			if (s->note == 0 && s->instrument == 0)
			{
				...
				else
				{
					xm_trigger_note(
						ctx, ch,
						flags | XM_TRIGGER_KEEP_PERIOD | XM_TRIGGER_KEEP_SAMPLE_POSITION);
				}
			}
			break;
	...
	for (uint8_t i = 0; i < ctx->module.num_channels; ++i)
	{
		...
		case 27: /* Rxy: Multi retrig note */
			...
			if ((ctx->current_tick % (ch->multi_retrig_param & 0x0F)) == 0)
			{
				...
				{
					float v = ch->volume * multi_retrig_multiply[ch->multi_retrig_param >> 4] + multi_retrig_add[ch->multi_retrig_param >> 4] / (float)0x40;
					XM_CLAMP(v);
					ch->volume = v;
				}
			}
...
static int xm_sample(xm_context_t *ctx, float *left, float *right)
{
...
int xm_generate_samples(xm_context_t *ctx, float *output, size_t numsamples)
{
	...
	{
		int res = xm_sample(ctx, output + (2 * i), output + (2 * i + 1));
